...
class Ship(Sprite):
    ...
    def update(self):
        ...
        if not self.ai_settings.ship_destroyed:
            ...
            if self.cur_frame_death >= self.max_frame_death:
                if self.image == self.image2 or self.image == self.image1:
                    self.image = self.image3
                elif self.image == self.image3:
                    self.image = self.image2
                self.cur_frame_death -= self.max_frame_death
